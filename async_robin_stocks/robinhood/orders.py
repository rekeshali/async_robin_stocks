...
async def find_stock_orders(client, **arguments):
    ...
    if 'symbol' in arguments.keys():
        arguments['instrument'] = await get_instruments_by_symbols(
...
async def order_option_spread(client, direction, price, symbol, quantity, spread, account_number=None, timeInForce='gtc', jsonify=True):
    ...
    legs = []
    for each in spread:
        optionID = await id_for_option(symbol,
                                        each['expirationDate'],
                                        each['strike'],
                                        each['optionType'])
...
async def order_crypto(client, symbol, side, quantityOrPrice, amountIn="quantity", limitPrice=None, timeInForce="gtc", jsonify=True):
    ...
    :param amountIn: If left async default value of 'quantity', order will attempt to trade cryptos by the amount of crypto \
