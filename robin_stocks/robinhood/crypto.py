...
@login_required
async def get_crypto_positions(client, info=None):
...
SYMBOL_TO_ID_CACHE = {}
async def get_crypto_id(client, symbol):
    ...
    id = await get_crypto_info(client, symbol, 'id')
    if id:
...
@login_required
async def get_crypto_quote(client, symbol, info=None):
...
@login_required
async def get_crypto_quote_from_id(client, id, info=None):
...
@login_required
async def get_crypto_historicals(client, symbol, interval='hour', span='week', bounds='24_7', info=None):
    ...
    id = await get_crypto_info(client, symbol[0], info='id')
